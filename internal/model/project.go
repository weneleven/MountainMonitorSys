...
type Project struct {
	...
	ProjectShortName     string `gorm:"type:varchar(50);not null" json:"project_short_name"  label:"项目简称"`    //项目简称 最多50字
	ProjectType          string `gorm:"type:varchar(50);not null" json:"project_type"  label:"项目类型"`                   //项目类型
	Location             string `gorm:"type:varchar(100);not null" json:"location" label:"项目位置"`                      //项目位置
	...
	OfflinePushFrequency int    `gorm:"type:int;DEFAULT:86400" json:"offline_push_frequency"  label:"离线推送频率(s)"` //离散推送频率,单位秒
	MapScale             int    `gorm:"type:int" json:"map_scale"  label:"地图比例"`                                 //地图比例
	...
	UserID               uint   `gorm:"not null" json:"user_id"  label:"用户ID"`                                         //所属用户ID
}
